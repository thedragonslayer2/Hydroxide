...
local constants = { fadeLength = TweenInfo.new(0.15), textWidth = Vector2.new(1.33742e6, 20) }
...
function ContextMenuButton.new(icon, text)
	...
	instance.MouseEnter:Connect(function() enterAnimation:Play() end)
	instance.MouseLeave:Connect(function() leaveAnimation:Play() end)
...
function ContextMenuButton.setIcon(contextMenuButton, newIcon) contextMenuButton.Instance.Icon.Image = newIcon end

function ContextMenuButton.setText(contextMenuButton, newText) contextMenuButton.Instance.Label.Text = newText end

function ContextMenuButton.setCallback(contextMenuButton, callback)
	if not contextMenuButton.Callback then contextMenuButton.Callback = callback end
end
...
function ContextMenu.new(contextMenuButtons)
	...
	for _i, contextMenuButton in pairs(contextMenuButtons) do
		...
		if buttonWidth > instanceWidth then instanceWidth = buttonWidth end
...
function ContextMenu.add(contextMenu, contextMenuButton) table.insert(contextMenu.Buttons, contextMenuButton) end

function ContextMenu.show(contextMenu)
	if currentContextMenu then currentContextMenu:Hide() end
