...
local function scan(query)
	...
	for _i, v in pairs(getGc()) do
		if type(v) == "function" and not isXClosure(v) then
			...
			if typeof(script) == "Instance" and not scripts[script] and script:IsA"LocalScript" and script.Name:lower():find(query) and getScriptClosure(script) and pcall(function() getsenv(script) end) then 
